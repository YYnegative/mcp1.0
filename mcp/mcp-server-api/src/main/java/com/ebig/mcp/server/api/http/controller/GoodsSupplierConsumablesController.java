...
/**
 * @description:供应商耗材接口类
 * @author: wenchao
 * @time: 2019-10-15 14:29
 */
@RestController
@RequestMapping("/consumables/supplierconsumables")
@Slf4j
public class GoodsSupplierConsumablesController extends GoodsSupplierController {


    @Autowired
    private GoodsSupplierConsumablesService supplierConsumablesService;

    @Autowired
    private SysDictService sysDictService;

    @PostMapping("/save")
    public Hdi save(@RequestBody List<GoodsSupplierConsumablesVo> list, HttpServletRequest request) {
        try {
            //查询出存储方式
            ...
            if (CollectionUtils.isNotEmpty(dects) && CollectionUtils.isNotEmpty(goodsUnits)) {
                ...
                supplierConsumablesService.batchSave(list, supplierInfo);
                return Hdi.ok();
            }
