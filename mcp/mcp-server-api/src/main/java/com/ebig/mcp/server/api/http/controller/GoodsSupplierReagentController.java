...
/**
 * @description:供应商试剂接口类
 * @author: wenchao
 * @time: 2019-10-15 14:29
 */
@RestController
@RequestMapping("/reagent/supplierreagents")
@Slf4j
public class GoodsSupplierReagentController extends GoodsSupplierController {

    @Autowired
    private GoodsSupplierReagentService supplierReagentService;

    @Autowired
    private SysDictService sysDictService;

    @PostMapping("/save")
    public Hdi save(@RequestBody List<GoodsSupplierReagentVo> list, HttpServletRequest request) {
        try {
            //查询出存储方式
            ...
            if (CollectionUtils.isNotEmpty(dects)) {
                ...
                supplierReagentService.saveBatch(list, supplierInfo);
                return Hdi.ok();
            }
